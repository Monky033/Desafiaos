...
router.get("/products/:productId", async (req, res) => {
  try {
    ...
    res.status(500).render("error", {
      message: "Error al recuperar los detalles del producto.",
    });
  }
});
...
router.post("/carts/:cartId/products/:productId", async (req, res) => {
  try {
    ...
    if (!cart) {
      return res
        .status(404)
        .send("Carrito no encontrado o no pertenece al usuario.");
    }
    ...
    res
      .status(200)
      .send({ message: "Producto añadido al carrito", cartId: cart._id });
  } catch (error) {
